// Fluid “splash cursor” overlay (vanilla JS) for this static portfolio.
// Creates a fixed, pointer-events:none WebGL canvas that reacts to pointer movement.
// Auto-initializes on DOMContentLoaded; you can also call window.initSplashCursor({...}).
...
(function () {
  ...
  function initSplashCursor(overrides = {}) {
    if (document.getElementById('splash-cursor-root')) return;
    if (prefersReducedMotion()) return;

    const root = document.createElement('div');
    root.id = 'splash-cursor-root';
    ...
    const config = { ...DEFAULTS, ...overrides, PAUSED: false };
    ...
    function addKeywords(source, keywords) {
      ...
      let out = '';
      keywords.forEach((k) => (out += `#define ${k}\n`));
      return out + source;
    }
...
void main () {
  ...
  gl_FragColor = vec4(v, 0.0, 1.0);
    ...
    function generateColor() {
      // Grayscale palette (white/grey only)
      // Keep values fairly bright since the site background is dark and the canvas is transparent.
      const v = 0.75 + Math.random() * 0.25; // 0.75 → 1.0
      return { r: v, g: v, b: v };
    }
    ...
    function clickSplat(pointer) {
      const c = generateColor();
      pointer.color = [c.r * 10.0, c.g * 10.0, c.b * 10.0];
  ...
  document.addEventListener('DOMContentLoaded', function () {
    // Default init; customize by calling window.initSplashCursor({...}) before DOMContentLoaded.
